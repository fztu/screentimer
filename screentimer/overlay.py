...
class LockOverlay:
    ...
    def show(self):
        self._visible = True
        self._title_label.config(text="Time for a break!")
    ...
    def update_config(self, auto_unlock_secs: int, on_password_unlock):
        self._auto_unlock_secs = auto_unlock_secs
        self._on_password_unlock = on_password_unlock

    def show_daily_limit(self):
        """Show overlay with daily limit message instead of break message."""
        self._title_label.config(text="Daily screen limit reached!")
        self._countdown_label.config(text="Come back tomorrow \U0001f31f")
        self._pw_entry.delete(0, tk.END)
        self._error_label.config(text="")
        w = self._win
        w.geometry(f"{w.winfo_screenwidth()}x{w.winfo_screenheight()}+0+0")
